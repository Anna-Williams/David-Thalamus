...
#cellcalling has been done by CellRanger. In Cellranger version 7 the cellcalling
#is done with a very similar algorithm to dropletutilities one.

# symlincs have been created between main directory and data/cellranger
...
matrices <- here("data/CellRanger/", samples, "filtered_feature_bc_matrix")
metadata <- read.csv(here("data/metadata_scRNAseq.csv"))
metadata <- metadata[metadata$tissue == "thalamus", ]
...
# Save the object
saveRDS(sce, here("processed", "sce_th.RDS"))
