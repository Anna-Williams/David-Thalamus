...
```{r load-sce}
sce <- readRDS(here("processed", "sce_th.RDS"))
project <- "fire-mice"
```
...
For the vold dataset the sample names had the genotype included. That was helpful, so I will add it now here too. 

```{r}
sce$THn <- sce$Sample
sce$Sample <- paste0(sce$genotype, tolower(sce$THn))
```
...
far as it is roughly normal. Below we use it with the ratio between the number
...
#### What are these cells with low number of genes?
```{r}
sce_lowratio <- sce[,sce$ratio_detected_sum<0.063]
head(sort(rowSums(counts(sce_lowratio)), decreasing = T))

```

The high content of haemoglobin indicates it is red blood cells. 

## Thresholds for cell filtering 

I keep the outlier thresholds, these thresholds will be further adjusted after the
cluster QC.
